...
def summarise(age, class_of_worker, det_ind_code, det_occ_code, marital_stat,
    ...
    WHERE age = ? AND class_worker = ? AND data.det_ind_code = ? AND data.det_occ_code = ?
    """
    if marital_stat is not None:
        filter_query = filter_query + " AND marital_stat = ?"
    if major_ind_code is not None:
        filter_query = filter_query + " AND major_ind_code = ?"
    if major_occ_code is not None:
        filter_query = filter_query + " AND major_occ_code = ?"
    if hisp_origin is not None:
        filter_query = filter_query + " AND hisp_origin = ?"
    if sex is not None:
        filter_query = filter_query + " AND sex = ?"
    
    filter_query = filter_query + """)SELECT avg(wage_per_hour) as mean_wage, avg(weeks_worked) as mean_weeks_worked, 
       ...
       FROM filter;"""
    cursor.execute(filter_query, [age, class_of_worker, det_ind_code, det_occ_code, marital_stat,major_ind_code, major_occ_code, hisp_origin, sex])
    db.commit()
    return True